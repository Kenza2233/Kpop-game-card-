    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-card-bg/80 backdrop-blur-xl border-t border-white/10 px-6 py-4 md:top-0 md:bottom-auto md:border-b md:border-t-0 shadow-2xl">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        <div className="hidden md:block">
          <Link href="/" className="text-2xl font-black kpop-gradient bg-clip-text text-transparent italic tracking-tighter">
            KPOP GACHA
          </Link>
        </div>
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center gap-2 font-black transition-all px-4 py-2 rounded-xl ${
                    isActive
